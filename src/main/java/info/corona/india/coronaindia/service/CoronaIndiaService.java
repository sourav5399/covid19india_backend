...
public class CoronaIndiaService {
	...
	public Optional<CoronaHistoricalDataDO> getAllHistoricalDataforIndia() {
		...
		List<CoronaHistoricalDataDO> dataList = Arrays.asList(
				restTemplate.exchange(endPointUrl, HttpMethod.GET, entity, CoronaHistoricalDataDO[].class).getBody());
		Stream<CoronaHistoricalDataDO> indiaData = dataList.stream()
				.filter(data -> data != null && data.getCountry().equalsIgnoreCase("India"));
